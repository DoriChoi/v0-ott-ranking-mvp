...
import { sampleWeeklyData, sampleCategoryData, sampleMostPopular } from "@/lib/sample-data"
...
export default function HomePage() {
  ...
  const weeklyData = {
    ...
    weekEnd: "2025-02-02",
    unifiedTop100: sampleWeeklyData,
    globalByQuadrant: {
      tvEnglish: sampleCategoryData["TV (English)"],
      tvNonEnglish: sampleCategoryData["TV (Non-English)"],
      filmsEnglish: sampleCategoryData["Films (English)"],
      filmsNonEnglish: sampleCategoryData["Films (Non-English)"],
    },
  }
  ...
  const countryData = {
    weekStart: "2025-01-27",
    weekEnd: "2025-02-02",
    items: sampleWeeklyData.slice(0, 10),
  }

  const popularData = {
    tvEnglish: sampleMostPopular[2].items,
    tvNonEnglish: sampleMostPopular[3].items,
    filmsEnglish: sampleMostPopular[0].items,
    filmsNonEnglish: sampleMostPopular[1].items,
  }
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{`${weeklyData.weekStart} ~ ${weeklyData.weekEnd}`}</p>
              </div>
          ...
          <div>
            <div className="mb-4">
              ...
              <p className="text-sm text-muted-foreground">
                영화/TV × 영어/비영어 4개 카테고리 • {`${weeklyData.weekStart} ~ ${weeklyData.weekEnd}`}
              </p>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{`${countryData.weekStart} ~ ${countryData.weekEnd}`}</p>
              </div>
